...
if(!link.endsWith('/')&&(!link.endsWith('.php')&&!link.endsWith('.htm')&&!link.endsWith('.html')&&!link.endsWith('.asp')&&!link.includes('ISBN'))&&!link.includes('?')){link=`${link}/`}


srclink.push(link);srcacs.push(accdt);if(!rel){rel='nofollow';}linkrel.push(rel);if(!lic){lic='';}lcs.push(lic);}function refPrint(){let an=document.getElementsByClassName('href-ref'),li,co,dat,tgl,bln,thn,dafpus=document.getElementById('daf-pus'),cox=0;for(co=0;co<an.length;co++){if(srclink.includes(srclink[co])&&srclink.indexOf(srclink[co])!=co){continue;}else{dafpus.insertAdjacentHTML('beforeend','<li></li>');}}for(co=0;co<an.length;co++){li=document.querySelectorAll("#daf-pus > li");dat=new Date(srcacs[co]);tgl=dat.getDate();bln=dat.getMonth();bltxt=bulan[bln];thn=dat.getFullYear();jam=dat.getHours();if(jam<10){jam=`0${jam}`;}mnt=dat.getMinutes();if(mnt<10){mnt=`0${mnt}`;}

if(srclink.includes(srclink[co])&&srclink.indexOf(srclink[co])!=co){
    an[co].href=`#ref${srclink.indexOf(srclink[co])+1}`;
    if(!an[co].innerHTML){
        an[co].innerHTML=`<span id='point${srclink.indexOf(srclink[co])+1}'></span><sup>[${srclink.indexOf(srclink[co])+1}]</sup>`;
    }else{
        an[co].innerHTML=`<span id='point${srclink.indexOf(srclink[co])+1}'></span>${an[co].innerHTML}<sup>[${srclink.indexOf(srclink[co])+1}]</sup>`;
    }continue;
}else{
    ...
    if(lcs[co]){
        li[cox].insertAdjacentHTML('beforeend',` Lisensi: ${lcs[co]}`);
    }
    an[co].href=`#ref${cox+1}`;
    if(!an[co].innerHTML){
        an[co].innerHTML=`<span id='point${cox+1}'></span><sup>[${cox+1}]</sup>`;
    }else{
        an[co].innerHTML=`<span id='point${cox+1}'></span>${an[co].innerHTML}<sup>[${cox+1}]</sup>`;
    }
        cox++;
    }
}
